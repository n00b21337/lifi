import * as lifiDataTypes from "@lifi/types";
...
const run = async () => {
  ...
  try {
    ...
    const stakeTxData = encodeFunctionData({
      ...
      args: [
        config.swarmBatchOwner,
        config.swarmBatchInitialBalance,
        config.swarmBatchDepth,
        config.swarmBatchBucketDepth,
        config.swarmBatchNonce,
        config.swarmBatchImmutable,
      ],
    });
